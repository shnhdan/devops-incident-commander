...
def generate_logs(count=10000):
    ...
    services = ["api-gateway", "payment-service", "user-service", "auth-service", "order-service"]
    hosts = ["prod-01", "prod-02", "prod-03", "prod-04", "prod-05"]
    
    error_patterns = [
        ("DatabaseConnectionError", "CRITICAL", "Unable to connect to database - connection pool exhausted"),
        ("HighLatency", "ERROR", "Request timeout after 30s - downstream service unreachable"),
        ("OutOfMemory", "CRITICAL", "Java heap space exhausted - application requires restart"),
